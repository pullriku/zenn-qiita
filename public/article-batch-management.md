...
<details><summary>Zennのマークダウンと違うところ</summary>
...
<details><summary>Fromtmatterとは</summary>
...
`ParsedMd`を対応するプラットフォームのマークダウンに変換することを **コンパイル** と呼んでいます。[^zeta.inline.2] `ZennCompiler`と`QiitaCompiler`によって、プラットフォームごとの記法に変換されます。比較しやすいように、まずはFrontmatterの変換を見てみましょう。
...
[^zeta.inline.2]: ここまでの一連の流れがプログラムのコンパイルと似ているためです。
